    ...
    <header className={`navbar${scrolled ? " navbar--scrolled" : ""}${menuOpen ? " navbar--open" : ""}`}>
      <nav className="navbar__inner">
        ...
        <div className="navbar__links">
          {NAV_LINKS.map(link => (
            <Link key={link} href={getPath(link)} className="navbar__link">{link}</Link>
          ))}

          <div className="navbar__dropdown-wrap" ref={dropdownRef}>
            <button
              id="activities-dropdown-btn"
              className={`navbar__dropdown-trigger${dropdownOpen ? " navbar__dropdown-trigger--open" : ""} navbar__dropdown-trigger--active`}
      ...
      <div className={`navbar__mobile-menu${menuOpen ? " navbar__mobile-menu--open" : ""}`}>
        {NAV_LINKS.map(link => (
          <Link key={link} href={getPath(link)} className="navbar__mobile-link" onClick={() => setMenuOpen(false)}>{link}</Link>
        ))}
        ...
        {ACTIVITY_DROPDOWN_ITEMS.map(item => (
          <Link key={item.type}
            ...
            className="navbar__mobile-sublink" onClick={() => setMenuOpen(false)}>
            {item.label}
          </Link>
        ))}
